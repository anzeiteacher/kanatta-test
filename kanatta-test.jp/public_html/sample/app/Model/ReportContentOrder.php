...
class ReportContentOrder extends AppModel
{
    ...
    public $belongsTo = array(
        'Report' => array(
            'className' => 'Report', 'foreignKey' => 'report_id',
    ...
    public function order_add($content)
    {
        ...
        if(!$order){
            ...
            $this->create();
            if($this->save($order_data)) return true;
        }else{
            //既にある場合は後ろに追加
    ...
    public function move_up($report_id, $data)
    {
        ...
        if($my_key === false || $ue_key === false) return false;
    ...
    public function move_down($report_id, $data)
    {
        ...
        if($my_key === false || $down_key === false) return false;
