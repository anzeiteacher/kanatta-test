...
class User extends AppModel
{
    ...
    {
        return $this->create_thumbnail($file_path, array(array(48, 48)));
    }
     ...
     * - キャンセル済みのBPは除く
     */
    public function get_backers($project_id, $mode = 'options', $limit = 10)
    {
        $options = array(
            'joins' => array(
                array(
            ...
            'fields' => array(
                ...
                'BackedProject.created', 'BackedProject.comment'
            )
        );
        if($mode == 'options') return $options;
        else return $this->find('all', $options);
    }
    ...
    public function reset_login_try_count($user_id)
    {
        ...
        $this->id = $user_id;
        if($this->save($user, true, array('login_try_count', 'token'))){
            return true;
        }
        return false;
    }
    ...
    {
        $this->id = $user_id;
        $data = array('active' => 0, 'twitter_id' => '', 'facebook_id' => '');
