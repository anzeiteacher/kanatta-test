...
class ProjectHelper extends AppHelper
{
    ...
    /**
     * 達成率の取得関数
     *  - 決済パターンが月額課金以外の場合、目標金額に対する達成率
     *  - 決済パターンが月額課金の場合、目標人数に対する達成率
     * @param array $project
     * @return int $per
     */
    public function get_backed_per($project)
    {
        if(!empty($project['Project']['no_goal'])){
            return 0;
        }
        switch($project['Project']['pay_pattern']){
            case ALL_OR_NOTHING:
            case ALL_IN:
                ...
                break;
            case MONTHLY:
                if(empty($project['Project']['backers'])){
                    return 0;
                }else{
                    return (int)(($project['Project']['backers'] / $project['Project']['goal_backers']) * 100);
                }
        }
        return 0;
    }
     ...
     * 決済パターンが月額課金かを返す
     */
    public function chk_pay_monthly($pj)
    {
    ...
    /**
     * プロジェクトの決済パターンに応じた注意文を返す
     */
    public function msg_of_pay_pattern($pattern)
    {
        switch($pattern)
        {
            case ALL_OR_NOTHING:
                return ALL_OR_NOTHING_MSG;
            case ALL_IN:
                return ALL_IN_MSG;
            case MONTHLY:
                return MONTHLY_MSG;
