...
class UsersController extends AppController
{
    ...
    public function view($user_id = null, $mode = null)
    {
        ...
        if(!$user || !$user['User']['active']) $this->redirect('/');
        ...
        if(!$mode || $mode == 'backed'){ //ユーザーが特定サイトで支援したプロジェクト一覧
            ...
            //プロジェクト取得
            $this->paginate = $this->Project->get_back_pj_of_user($user_id, 'options', 10);
            ...
            //作成したプロジェクト一覧取得
            $this->paginate = $this->Project->get_pj_of_user($user_id, 'options', 10);
            $this->set('projects', $this->paginate('Project'));
    ...
    public function edit()
    {
        ...
        if($this->request->is('post') || $this->request->is('put')){
            ...
            if($this->User->save($this->request->data, true, array(
                'name', 'nick_name', 'address', 'self_description',
                'url1', 'url2', 'url3', 'receive_address', 'img',
                'birth_area', 'school', 'tw_screen_name', 'fb_name'
            ))){
                $change_mail = false;
                if($this->auth_user['User']['email'] != $this->request->data['User']['email']){
                    if(empty($this->request->data['User']['email'])){
                        return json_encode(array('status' => 0, 'msg' => 'メールアドレスを入力してください'));
                    }
                    $change_mail = true;
                    $result = $this->change_email($this->request->data['User']['email'],
                        $this->Auth->user('id'));
                    if($result > 1){
                        if($result == 3){
                            $this->User->rollback();
                            return json_encode(array('status' => 0, 'msg' => 'このメールアドレスは既に登録されています'));
                        }elseif($result == 2){
                            $this->User->rollback();
                            return json_encode(array('status' => 0, 'msg' => 'メールの登録ができませんでした。'.OSORE));
                        }
                    }
                }
                $this->User->commit();
                ...
                if($change_mail){
                    return json_encode(array('status' => 0, 'msg' => 'メールアドレス認証用URLをお送りいたしました'));
                }else{
                    return json_encode(array('status' => 0, 'msg' => 'プロフィールを更新しました。'));
                }
            }
            $this->User->rollback();
            return json_encode(array('status' => 0, 'msg' => 'プロフィールが保存できませんでした。'.OSORE));
        }else{
    ...
    public function mypage()
    {
        ...
        //支援したプロジェクト一覧
        $this->paginate = $this->Project->get_back_pj_of_user($this->Auth->user('id'), 'options', 10);
    ...
    public function delete($user_id)
    {
        ...
        if($this->_chk_delete_enable($user_id)){
     ...
     * プロジェクトに支援していないか？
     * @params int $user_id
     * @return bool (trueなら退会可能)
     */
    private function _chk_delete_enable($user_id)
    {
