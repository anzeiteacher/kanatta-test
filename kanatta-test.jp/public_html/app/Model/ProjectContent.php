...
class ProjectContent extends AppModel
{
    ...
    public function movie_edit($data)
    {
        ...
        if($this->save($data, true, array('movie_type', 'movie_code'))){
            return $this->read();
        }else{
            return null;
        }
    ...
    public function content_open($project_content_id)
    {
        ...
        if($this->saveField('open', 1)) return true;
        return false;
    }
    ...
    {
        $this->id = $project_content_id;
        if($this->saveField('open', 0)) return true;
        return false;
    }
    ...
    public function open_all($project_id)
    {
        ...
        if($contents){
            foreach($contents as $c){
                ...
                if(!$this->saveField('open', 1)) $flag = false;
            }
        }
        if($flag) return true;
        return false;
