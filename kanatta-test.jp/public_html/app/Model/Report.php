...
class Report extends AppModel
{
    ...
    public function get_open_reports_options_of_project($project_id, $limit = 15, $recursive = -1)
    {
        return array(
            ...
            'recursive' => $recursive,
            'fields' => array(
                'Report.id', 'Report.project_id', 'Report.title',
                'Report.created', 'Report.open', 'Project.user_id'
            )
        );
    }
    ...
    public function get_all_reports_options_of_user($user_id, $limit = 15)
    {
        return array(
            ...
            'limit' => $limit,
            'fields' => array(
                'Project.project_name', 'Project.id', 'Project.user_id',
                'Report.id', 'Report.created', 'Report.title',
                'Report.open', 'Report.project_id'
            )
        );
    }
    ...
    public function open($report_id)
    {
        ...
        if(empty($report['Report']['title'])){
            return array(3, '公開にはタイトルの登録が必要です');
        }
        $this->id = $report_id;
        $this->begin();
    ...
    public function get_new_reports_in_site()
    {
        $options = array(
            ...
            'conditions' => array('Report.open' => 1, 'Project.stop' => 0),
            'order' => array('Report.created' => 'DESC'),
            'limit' => 10,
            'fields' => array(
                'Report.id', 'Report.project_id', 'Report.title',
