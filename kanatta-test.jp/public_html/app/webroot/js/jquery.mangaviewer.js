	...
	$.fn.extend({
		...
		mangaviewer:function(options){
			...
			o.view_obj_create_right =function(o, ejections){
				...
				if (o.options.page >= ejections["below_num"]) {
					$('#'+ejections["sheet_num"]).prepend('<img id="page'+ ejections["below_num"] +'" class="right_page" rel="' + ejections["next_page"] +'" src="' + o.options.path + '/' + ejections["below_num"] + '.' + o.options.ext + '">');
				}
				$('#'+ejections["sheet_num"]).prepend('<img id="page'+ ejections["above_num"] +'" class="left_page" rel="' + ejections["prev_page"] +'" src="' + o.options.path + '/' + ejections["above_num"] + '.' + o.options.ext + '">');
			...
			o.pager_create =function(o){
				...
				if ($('#page-list').children().size()) {
					...
					for(i=1;i<=max_view;i++){
						if (o.options.page_ejection == "right") {
