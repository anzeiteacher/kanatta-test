...
class ReportsController extends AppController
{
    public $uses = array(
        ...
        'BackedProject', 'FavouriteProject'
    );
    ...
    public function add($project_id = null)
    {
        ...
        if($this->request->is('post') || $this->request->is('put')){
            ...
            if($this->Report->save($this->request->data, true, array(
                ...
                $this->redirect(array('action' => 'edit', $this->Report->field('id')));
            }else{
    ...
    public function open($report_id)
    {
        ...
        switch($result[0]){
            case 1:
                ...
                $this->_mail_create_report($report_id);
                break;
            case 2:
    ...
    /**
     * 支援者とお気に入り登録者に活動報告作成通知メールを送信する
     * @param $report_id
     */
    private function _mail_create_report($report_id)
    {
        $report = $this->Report->findById($report_id);
        if($report['Report']['first_open_flag']) return;
        $pj_id = $report['Report']['project_id'];
        $pj = $this->Project->findById($pj_id);
        $backers = $this->BackedProject->get_backers_of_project_distinct($pj_id);
        $favo_users = $this->FavouriteProject->get_favo_users($pj_id);
        $url = h(Router::url('/', true)).'projects/view/'.$pj_id.'/report_view/'.$report_id;
        $backers_ids = [];
        foreach($backers as $b){
            $this->Mail->create_report($b, $pj, $report, $url);
            $backers_ids[] = $b['User']['id'];
        }
        foreach($favo_users as $f){
            if(in_array($f['User']['id'],  $backers_ids)) continue;
            $this->Mail->create_report($f, $pj, $report, $url);
        }
        $this->Report->saveField('first_open_flag', 1);
    }
