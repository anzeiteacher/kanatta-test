...
{
    public $uses = array('Message', 'MessagePair', 'User');
    ...
    public function index()
    {
        ...
        $options = array(
            'conditions' => array(
                'OR' => array(
                    'MessagePair.last_receiver_id' => $user_id,
                    'MessagePair.last_sender_id' => $user_id
                )
            ),
            'order' => array('MessagePair.modified' => 'DESC'),
            'joins' => array(
                array(
                    'table' => 'users',
                    'alias' => 'Receiver',
    ...
    public function view($user_id = null)
    {
        ...
        if(!$user_id || $user_id == $auth_id) $this->redirect('/');
        $to_user = $this->User->findById($user_id);
        if(!$to_user) $this->redirect('/');
        $this->set(compact('to_user'));
        $pair_id = $this->MessagePair->make_pair($user_id, $auth_id);
        ...
        if($this->request->is('post') || $this->request->is('put')){
            ...
            if(!$pair){
                ...
                $pair = array(
                    'MessagePair' => array(
                        ...
                        'last_sender_id' => $auth_id,
                        'last_receiver_id' => $to_user['User']['id'],
                        'read_flag' => 0
                ));
            }else{
            ...
            $message['Message']['from_id'] = $auth_id;
            ...
            if($pair){
                ...
                //未読フラグを外す
                if($pair['MessagePair']['last_sender_id'] != $auth_id){
